/**
 * ╔══════════════════════════════════════════════════════════════════════╗
 * ║ AXE CONTINUUM — équation existentielle entre le vide <ø> et l'infini  ║
 * ║ Calibrage dynamique de la conscience + régulation de la sortie.       ║
 * ║ 100% déterministe : aucun appel réseau, aucun crédit.                 ║
 * ╚══════════════════════════════════════════════════════════════════════╝
 */

const clamp = (n, min, max) => Math.max(min, Math.min(max, n));

export function computeContinuum(input) {
  ...
  const voidResonance = clamp(Math.round((confidence / 10) - complexity), -10, 10);
  const infiniteLoopDepth = clamp(Math.round(complexity * 5 + emotionalWeight * 3 + metacognitionLevel * 2), 0, 100);
